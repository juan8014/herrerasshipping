...
import type { DeliveryRow } from "@/lib/deliveries-data"
...
export function DeliveryBoard({
  ...
  departamento: string
  inCountry: DeliveryRow[]
  onTheWay: DeliveryRow[]
}) {
  ...
  function run(ids: string[], status: string, note: string) {
    startTransition(async () => {
      await updateShipmentsStatus({ ids, status, note, location: departamento })
      setSelected(new Set())
      router.refresh()
    })
  }
  ...
  return (
    <div className="space-y-6">
      {/* En el país — listos para despachar */}
      <section className="overflow-hidden rounded-2xl border border-[#7BB5E6]/20 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#7BB5E6]/10 px-5 py-4">
          <h2 className="font-semibold text-[#234974]">
            En el país <span className="text-[#234974]/50">({inCountry.length})</span>
          </h2>
          <Button
            onClick={() => run([...selected], "out_for_delivery", `En camino a ${departamento}`)}
        ...
        {inCountry.length === 0 ? (
          <div className="px-5 py-10 text-center text-[#234974]/50">
            ...
            No hay paquetes esperando reparto en {departamento}.
          </div>
        ) : (
              ...
              <tbody>
                {inCountry.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-[#234974]/60">{r.clients?.address ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="overflow-hidden rounded-2xl border border-[#7BB5E6]/20 bg-white shadow-sm">
        ...
        {onTheWay.length === 0 ? (
          <div className="px-5 py-10 text-center text-[#234974]/50">
            No hay paquetes en camino en {departamento}.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right">
                      <Button
                        ...
                        variant="outline"
                        disabled={pending}
                        onClick={() => run([r.id], "delivered", `Entregado en ${departamento}`)}
